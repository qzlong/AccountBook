...
public class TypeLabelGridLayout extends GridLayout {
    ...
    private void mulAddTabs(final FilterBean model){
        ...
        List<FilterBean.TableMode> labels = model.getLabels();
            ...
            label.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (label.isSelected()){
                        ...
                        if (label.getText().toString().equals("全部类别") ||
                            label.getText().toString().equals("一级分类") ||
                            label.getText().toString().equals("二级分类") ||
                            label.getText().toString().equals("全部收入") ||
                            label.getText().toString().equals("全部支出") ||
                            label.getText().toString().equals("其他全部")) {
                            resetData();
                        }else {
                            setAllLabel("基础");
                        }
                        ...
                        for (FilterBean.TableMode tab : selectTab) {
                            list.add(tab.getName());
                        }
                        Log.e("rcw","selectTab="+list);
                        selectLabel.add(label);
                    }
                    //记录选中的label
    ...
    private void setAllLabel(String title) {
        if ("基础".equals(title)) {
            setLabel("基础", "全部类别");
            setLabel("基础", "一级分类");
            setLabel("基础", "二级分类");
    ...
    private void setLabel (String title, String value) {
        for (TextView label :selectLabel) {
            if (label.getText().equals(value)) {
                label.setSelected(false);
            }
        }
        for (FilterBean.TableMode tab : selectTab) {
            if (tab.getName().equals(value)) {
                selectTab.remove(tab);
            }
        }
        for (String labelinfo : labelLists) {
            if (labelinfo.equals(title + "-" + value)) {
                labelLists.remove(labelinfo);
            }
        }
    }
    /**
    ...
    private boolean inTabList(FilterBean.TableMode tab0, List<FilterBean.TableMode> tabList) {
    ...
    public List<FilterBean.TableMode>  getTabList() {
        return selectTab;
    }
    public void setSelectLabel(List<FilterBean.TableMode> selectTab) {
