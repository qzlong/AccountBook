...
public class FilterBean implements Parcelable {
    ...
    private List<TableMode> tabs = new ArrayList<>(); //标签集合
    private List<TableMode> labels = new ArrayList<>(); //标签集合
    ...
    public void addLabel(TableMode tab) {
        if (this.labels.size() != 0) {
            for (TableMode label : this.labels) {
                if (label == tab) {
                    return;
                }
            }
